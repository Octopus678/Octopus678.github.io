...
- About intro: godly #159 style light cream panel with big outlined scrolling words; left = portrait in a device frame, right = React Bits Lanyard interactive 3D physics ID card (drag to swing / flip; front = intro card, back = photo). Photo showcase below: 1:1 replica of React Bits Circular Gallery — a WebGL curved strip of all 11 photos, draggable / wheel / arrow-key navigation, ripple wave animation; clicking a photo opens an in-page lightbox (no navigation).
...
- 介绍区（关于）：Godly #159 浅色米白面板 + 超大描边字竖排滚动；左侧设备屏人像照片，右侧 **React Bits Lanyard 3D 物理名片**（可拖动甩动、翻转；正面介绍卡、背面照片）。下方照片展：1:1 复刻 React Bits Circular Gallery —— WebGL 弧形画廊，全部 11 张照片沿弧线排布，可拖动 / 滚轮 / 方向键浏览，带波纹动效与逐图标题；点击照片在**站内弹层**中查看，不跳转。
