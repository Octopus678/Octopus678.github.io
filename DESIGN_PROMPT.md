...
- Background: fixed full-screen image of a red-and-black resin galaxy swirl (unchanged), plus a fixed full-page React Bits Molten Metal WebGL fluid layer (ember palette, mouse-follow, auto-pauses offscreen) blended over the scene.
...
- Hero (melius-style header): dark full-width nav bar with rounded bottom corners and hairline border; the hero canvas holds **all 14 clips as rounded, slightly rotated video tiles** (muted, looping, autoplay, pointer-events none — never clickable), with a left-to-right dark scrim so the headline stays readable; big two-line headline, primary CTA, platform marquee at the bottom.
- About intro: godly #159 style light cream panel with big outlined scrolling words; left = React Bits ProfileCard 3D tilt avatar card, right = React Bits Lanyard interactive 3D physics card (same size as the photo; drag to swing / flip; front = full intro card with paragraph & stats, back = photo). Photo showcase below: 1:1 replica of React Bits Circular Gallery — a WebGL curved strip of all 11 photos with continuous rotation/ripple, **no text labels under photos**, clicking a photo opens an in-page lightbox (no navigation).
...
- 背景：固定整屏使用「红黑树脂星系漩涡」图片（原背景保留），并叠加整页 React Bits Molten Metal WebGL 熔岩流体层（ember 配色、跟随鼠标、离屏自动暂停）。
...
- Hero（melius 风格头部）：深色全宽导航条（底部圆角 + 细描边）；头部画布内放入**全部 14 支成片**，做成圆角、略带旋转的视频卡片（静音循环自动播放、`pointer-events:none` 不可点击）；左侧到右侧的暗色渐变保证大标题可读；两行大标题 + 主 CTA + 底部平台跑马灯。
- 介绍区（关于）：Godly #159 浅色米白面板 + 超大描边字竖排滚动；左侧 **React Bits ProfileCard 3D 倾斜头像卡**，右侧 **React Bits Lanyard 3D 物理名片**（与照片同尺寸，可拖动甩动、翻转；正面为完整介绍+数据、背面照片）。下方照片展：1:1 复刻 React Bits Circular Gallery —— WebGL 弧形画廊，全部 11 张照片沿弧线排布、持续旋转波纹动效，**照片下方无文字标签**；点击照片在**站内弹层**中查看，不跳转。
