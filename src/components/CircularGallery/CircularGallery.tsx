// React Bits (MIT) — https://www.reactbits.dev/components/circular-gallery
// 适配：事件作用域限定在容器内；离屏自动暂停；支持点击打开
...
class App {
  ...
  onClick(e: MouseEvent) {
    if (this.dragDistance > 12) return; // 拖拽结束不触发
    ...
    if (best) {
      const original = best.index % this.itemCount;
      this.onItemClick(original);
    }
  }
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    let isMounted = true;
    // 自定义字体：确保宿主页已声明后直接使用，避免请求 Google Fonts
