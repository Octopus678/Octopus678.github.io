...
1. 全屏 Hero（melius 风格头部）：深色圆角导航条 + **14 支成片视频画布**，所有视频在头部循环播放（静音、自动、不可点击打开）、大标题、联系按钮、平台跑马灯
...
- **头部视频**：`public/videos/` 下的 14 支成片切片（`*.mp4` + `*.webm` + 同名 `.jpg` 封面），每支 12 秒 / 480p / 无音轨，由 `C:\Users\Administrator\Desktop\网站` 的源视频转码而来
